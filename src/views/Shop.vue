            ...
            <template v-for="(image, index) in images" :key="image.id">
                ...
                <div class="flex items-center justify-between border rounded-lg p-6 bg-white shadow-sm">
                    ...
                    <div class="flex flex-col items-center w-1/3">
                        ...
                        <img :src="getImageSrc(image.ref_image)" :alt="image.ref_image"
                            class="w-32 h-32 object-cover rounded-md mx-auto mb-2" />
                    </div>
                    <div class="flex flex-col items-center w-1/3">
                        <div class="text-lg font-semibold mb-2"
                        ...
                        <!-- <div class="text-lg font-semibold mb-2">{{ image.target_name }}</div> -->
                        <img :src="getImageSrc(image.recom_ref_image)" :alt="image.recom_ref_image"
                            class="w-32 h-32 object-cover rounded-md mx-auto mb-2" />
                    </div>
                    <!-- Matching Controls -->
                    <div class="flex flex-col items-center justify-center w-1/4 space-y-4">
                        ...
                        <div class="flex space-x-2">
                            <button @click="updateRefPID(image)"
                                class="px-4 py-2 rounded-md font-medium transition-colors" :class="image.ref_pid === image.recom_ref_pid
        ...
        async updateRefPID(image) {
            try {
                ...
                if (response.data.success) {
                    image.ref_pid = image.recom_ref_pid
                }
            } catch (error) {
